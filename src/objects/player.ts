...
export class Player extends Phaser.GameObjects.Image {
  ...
  private handleInput() {
    ...
    if (this.rotateKeyLeft.isDown) {
      if(this.scene.registry.get("audio") == 1)
      {
      this.audio.audioRotate.play();
      }
  ...
  private handleShooting(): void {
    if (this.shootingKey.isDown && this.scene.time.now > this.lastShoot) {
      if(this.scene.registry.get("audio") == 1)
      {
      this.audio.audioShoot.play();
      }
      this.scene.cameras.main.shake(2, 0.005);
  ...
  public updateHealth(): void {
    if (this.health > 0) {
      ...
      if(this.scene.registry.get('score') > this.scene.registry.get('highscore'))
      {
        this.scene.registry.set('highscore', this.scene.registry.get('score'));
      }
